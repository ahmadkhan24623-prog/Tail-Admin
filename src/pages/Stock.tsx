    ...
    <div className="p-4 md:p-6 space-y-6 min-h-screen text-slate-800 dark:text-slate-100 font-sans">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl p-6 md:p-8 text-white shadow-lg">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <button onClick={() => showToast('Inventory re-synced with warehouse nodes.')} className="flex-1 lg:flex-none bg-white/10 hover:bg-white/20 backdrop-blur-md text-white px-4 py-2.5 rounded-xl text-xs font-bold transition-all border border-white/20 flex items-center justify-center gap-2 cursor-pointer shadow-sm">
              <RefreshCw size={14} className="animate-spin-slow" /> Re-sync Node
            </button>
            <button onClick={() => showToast('Asset intake form is not available in this demo yet.', 'info')} className="flex-1 lg:flex-none bg-white text-blue-600 hover:bg-blue-50 font-black px-5 py-2.5 rounded-xl text-xs transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer">
              + New Asset Influx
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col justify-between">
          ...
          <button onClick={() => showToast('Sector loads rebalanced across all nodes.')} className="w-full py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition-all border border-slate-200 dark:border-slate-700 cursor-pointer">
